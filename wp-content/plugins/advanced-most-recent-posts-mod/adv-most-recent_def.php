...
function yg_recentposts($args = '', $echo = true) {
	...
	*/
		...
		foreach ($posts as $post) {
			...
			$postlist .= '<li>'.($spot == 'spot1' ? '<span class="date">'.$time.'</span> ' : '').'<a href="' . get_permalink($post->ID) . '" title="'. $post_title .'" >' . $image . $post_title .'</a>' . ($showauthor ? ' by '.get_the_author($post->post_author) : '') . '' . ($spot == 'spot2' ? ' <span class="date">'.$time.'</span>' : '') . $excerpt . "</li>";
