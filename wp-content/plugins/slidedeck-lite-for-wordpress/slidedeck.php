...
define( 'SLIDEDECK_VERSION',                        '1.3.3' );
define( 'SLIDEDECK_EXCERPT_LENGTH_WITH_IMAGE',      30 );
define( 'SLIDEDECK_EXCERPT_LENGTH_WITHOUT_IMAGE',   100 );
...
function slidedeck_menu() {
    add_menu_page( 'SlideDeck', 'SlideDeck', 2, basename( __FILE__ ), 'slidedeck_edit', slidedeck_url( '/images/icon.png' ) );
    add_submenu_page( basename( __FILE__ ), 'Edit SlideDeck', 'Edit', 2, basename( __FILE__ ), 'slidedeck_edit' );
    add_submenu_page( basename( __FILE__ ), 'Add SlideDeck', 'Add New', 2, basename( __FILE__ ) . '/slidedeck_add_new', 'slidedeck_add_new' );
    add_submenu_page( basename( __FILE__ ), 'Add Smart SlideDeck', 'Add Smart SlideDeck', 2, basename( __FILE__ ) . '/slidedeck_dynamic', 'slidedeck_dynamic' );
    add_submenu_page( basename( __FILE__ ), 'Buy SlideDeck Pro', 'Buy SlideDeck Pro', 2, basename( __FILE__ ) . '/upgrade', 'slidedeck_upgrade' );
}
...
function slidedeck_admin_init() {
    ...
    if ( slidedeck_is_plugin() ) {
        if( $_GET['page'] == basename( __FILE__ ) . "/upgrade" ) {
            ...
            wp_redirect( "http://www.slidedeck.com/pricing" );
        }
...
function slidedeck_skin_meta( $skin_file ) {
    ...
    if ( !empty( $meta_raw ) ) {
        ...
        $skin = array(
            'url' => slidedeck_url( substr( $skin_file, strpos( $skin_file, "/skins" ) ) ),
            'thumbnail' => slidedeck_url( substr( $skin_folder, strpos( $skin_folder, "/skins" ) ) . "/thumbnail.png" ),
            'slug' => $skin_slug,
            'meta' => $skin_meta
        );
        if( file_exists( $skin_folder . "/skin.ie.css" ) ) {
            $skin['ie_url'] = slidedeck_url( substr( $skin_folder, strpos( $skin_folder, "/skins" ) ) . "/skin.ie.css" );
        }
        if( file_exists( $skin_folder . "/skin.ie7.css" ) ) {
            $skin['ie7_url'] = slidedeck_url( substr( $skin_folder, strpos( $skin_folder, "/skins" ) ) . "/skin.ie7.css" );
        }
        if( file_exists( $skin_folder . "/skin.ie8.css" ) ) {
            $skin['ie8_url'] = slidedeck_url( substr( $skin_folder, strpos( $skin_folder, "/skins" ) ) . "/skin.ie8.css" );
        }
        if ( file_exists( $skin_folder . "/skin.js" ) ) {
            $skin['script_url'] = slidedeck_url( substr( $skin_folder, strpos( $skin_folder, "/skins" ) ) . "/skin.js" );
        }
        ...
        foreach( $images as $image ) {
            ...
            if( $output_set === false ) {
                ...
                if( (boolean) $validate === true ) {
                    ...
                    if( preg_match( '/(tweetmeme|stats|advertisement|commindo|valueclickmedia|imediaconnection|adify|traffiq|premiumnetwork|advertisingz|gayadnetwork|vantageous|networkadvertising|advertising|digitalpoint|viraladnetwork|decknetwork|burstmedia|doubleclick).|feeds\.[a-zA-Z0-9\-_]+\.com\/~ff|wp\-digg\-this|feeds\.wordpress\.com/i', $image['src'] ) ) {
                        $valid = false;
                    }
                }
 ...
 * @return Rendered SlideDeck markup and JavaScript tag to initialize SlideDeck render
 */
function slidedeck( $slidedeck_id, $styles=array( 'width' => '100%', 'height' => '300px' ) ) {
