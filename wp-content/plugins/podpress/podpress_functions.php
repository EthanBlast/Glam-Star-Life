...
*/
	...
	*/
	...
	function podPress_feedButtons_control() {
		...
		<div id="podpress_widget_accordion" class="podpress_widget_accordion"><!-- Begin: podPress Widget Accordion -->
	...
	function podPress_xspfPlayer_control() {
		...
		// print error and other message from the XSPF automatic skin file modification process
		podpress_print_xspf_log();
		?>
	...
	*/
	...
	function podPress_processDownloadRedirect($postID, $mediaNum, $filename, $method = '') {
		...
		if($podPress->settings['enable3rdPartyStats'] == 'PodTrac') {
			...
			printphpnotices_var_dump('starting full+ download');
			if ($handle = fopen($realSysPath, 'rb')) {
				while (!feof($handle) && connection_status()==0) {
					echo fread($handle, $chunksize);
					ob_flush();
					flush();
					printphpnotices_var_dump($i++);
				}
				fclose($handle);
			}
			printphpnotices_var_dump(connection_status());
			printphpnotices_var_dump(connection_aborted());
			...
			printphpnotices_var_dump('ending full+ download');
			exit;
		}
