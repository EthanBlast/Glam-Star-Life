		...
		foreach ($podPressTemplateData['files'] as $key=>$val) {
			...
			if($val['enablePlayer']) {
				...
				if ( 'audio_mp3' == $val['type'] AND TRUE == isset($podPress->settings['player']['listenWrapper']) AND TRUE == $podPress->settings['player']['listenWrapper'] AND FALSE == $podPress->settings['enablePodangoIntegration'] AND TRUE == isset($podPress->settings['mp3Player']) AND '1pixelout' == $podPress->settings['mp3Player'] ) {
					$podPressContent .= "\n".'<div class="podpress_listenwrapper_container" id="podpress_lwc_'.$GLOBALS['podPressPlayer'].'" style="background-image:url('.PODPRESS_URL.'/images/listen_wrapper.gif);"><div class="podpress_mp3_borderleft"></div><div class="podpress_1pixelout_container"><div id="podPressPlayerSpace_'.$GLOBALS['podPressPlayer'].'"></div></div></div>'."\n";
					...
					$podPressContent .= "\n".'<div class="podpress_playerspace podpress_mp3player"><div id="podPressPlayerSpace_'.$GLOBALS['podPressPlayer'].'"></div></div>'."\n";
				} else {
					$podPressContent .= "\n".'<div class="podpress_playerspace" id="podPressPlayerSpace_'.$GLOBALS['podPressPlayer'].'"></div>'."\n";
				}
			}
			...
			if(isset($val['image'])) {
				if($val['enableDownload'] && !empty($val['URI'])) {
					if ( 0 === strpos($val['type'], 'embed_') ) {
						$podPressDownloadlinks .= '<a href="'.$val['URI'].'" target="new" title="'.sprintf(__('Direct Link to %1$s', 'podpress'), $podPressEpisodeTitle).'">';
					} else {
						$podPressDownloadlinks .= '<a href="'.$val['URI'].'" target="new" title="'.sprintf(__('Download: %1$s', 'podpress'), $podPressEpisodeTitle).'">';
					}
				}
			...
			if(TRUE == isset($val['enableTorrentDownload']) AND TRUE === $val['enableTorrentDownload']) {
				$podPressDownloadlinks .= '<a href="'.$val['URI_torrent'].'" target="new" title="'.sprintf(__('Download: %1$s - .torrent file', 'podpress'), $podPressEpisodeTitle).'">';
			...
			if($podPressTemplateData['showDuration'] == 'enabled' && !empty($val['duration']) && preg_match("/([0-9]):([0-9])/", $val['duration'])) {
				$podPressDownloadlinks .= ' ['.$val['duration'].__('m', 'podpress').']';
			}
			...
			if(!$val['authorized']) {
				...
				if($val['enablePlayer']) {
					...
					if ($val['enablePlaylink']) {
						$podPressDownloadlinks .= '<a href="#podPressPlayerSpace_'.$GLOBALS['podPressPlayer'].'" onclick="javascript:podPressShowHidePlayer(\''.$GLOBALS['podPressPlayer'].'\', \''.$val['URI_Player'].'\', '.strval(intval($val['dimensionW'])).', '.strval(intval($val['dimensionH'])).', \'false\', \''.$val['previewImage'].'\', \''.js_escape($val['title']).'\', \''.js_escape($val['artist']).'\'); return false;"><span id="podPressPlayerSpace_'.$GLOBALS['podPressPlayer'].'_PlayLink" class="podPress_playerspace_playlink">'.$hideplayerplaynow_divider.__('Play Now', 'podpress').'</span></a>';
						$dividerNeeded = true;
					} else {
				...
				if($val['enableDownload'] && $podPressTemplateData['showDownloadText'] == 'enabled') {
					...
					if ( 0 === strpos($val['type'], 'embed_') ) {
						$podPressDownloadlinks .= '<a href="'.$val['URI'].'" target="new">'.__('Direct Link', 'podpress').'</a>';
						$val['stats'] = false;
					} else {
						$podPressDownloadlinks .= '<a href="'.$val['URI'].'" target="new">'.__('Download', 'podpress').'</a>';
