	...
	class podPressUpgrade_class extends podPress_class	
	{
		function podPressUpgrade_class($current = '0') {
			...
			if ( TRUE === version_compare(PODPRESS_VERSION, $current, '<=') ) {
				// if the version in the DB is the same as the version of this plugin then an upgrade is not nescessary
				$this->podPress_class();
				return;
			}
			...
			//~ printphpnotices_var_dump('before activation from update');
			if ( FALSE === $current OR '0' == $current ) { // if no version number was in the db or if something is wrong with it.
				//~ printphpnotices_var_dump('is the first installation');
				//~ printphpnotices_var_dump('current');
				//~ printphpnotices_var_dump($current);
				require_once(ABSPATH.PLUGINDIR.'/podpress/podpress_admin_functions.php');
				$this->activate();
			//~ } else {
				//~ printphpnotices_var_dump('is not the first installation');
				//~ printphpnotices_var_dump('current');
				//~ printphpnotices_var_dump($current);
			}
			...
			//~ printphpnotices_var_dump('the previous version was (the version number in the db is): ' . $current);
			require_once(ABSPATH.PLUGINDIR.'/podpress/podpress_admin_functions.php');
			...
			//~ printphpnotices_var_dump('the new version is: ' . $current);
