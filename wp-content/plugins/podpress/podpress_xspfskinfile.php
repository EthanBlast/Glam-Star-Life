...
if (FALSE !== strpos((__FILE__), 'wp-content')) {
	...
	if ( 1 < count($script_name_parts)) {
		...
		if ( TRUE == version_compare($wp_version, '2.8', '>=') ) {
			...
			//~ printphpnotices_var_dump('skin file: '.$podpress_xspf_widget_temp);
			...
			//~ printphpnotices_var_dump('skin file');
			//~ printphpnotices_var_dump($widget_id);
			//~ printphpnotices_var_dump($options['PlayerWidth'] . ' x ' . $options['PlayerHeight']);
		} else {
		...
		if ( TRUE == isset($options['useSlimPlayer']) AND TRUE === $options['useSlimPlayer'] ) {
			if ( FALSE == isset($options['SlimPlayerHeight']) OR 150 > intval($options['SlimPlayerHeight']) ) {
				$options['SlimPlayerHeight'] = 150; // min width
			}
			echo podpress_xspf_jukebox_slim_skin_xml($options['PlayerWidth'], $options['SlimPlayerHeight'], $blog_id);
		} else {
			if ( FALSE == isset($options['PlayerHeight']) OR 150 > intval($options['PlayerHeight']) ) {
				$options['PlayerHeight'] = 150; // min width
			}
