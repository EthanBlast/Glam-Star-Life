...
*/
	...
	function podPress_getCategoryCastingFeedData ($selection, $input) {
		...
		//~ printphpnotices_var_dump('### podPress_getCategoryCastingFeedData');
		//~ printphpnotices_var_dump($selection);
		$feedslug = get_query_var('feed');
		//~ printphpnotices_var_dump($feedslug);
		$cat_id = get_query_var('cat');
		//~ printphpnotices_var_dump($cat_id);
		...
		//~ printphpnotices_var_dump('is_podpress_feed: ');
		//~ printphpnotices_var_dump($is_podpress_feed);
		//~ //~ printphpnotices_var_dump($podPress->settings['category_data']['categoryCasting']);

		if ( empty($feedslug) || (FALSE === $podPress->settings['category_data'] AND FALSE === $is_podpress_feed) ) {
			return $input;
		} else {
			if ( 'true' === $podPress->settings['category_data']['categoryCasting'] ) {
		//~ printphpnotices_var_dump('is_category_feed');
	...
	*/
	...
	/**
	* podpress_only_posts_with_certain_files - filters $wp_query and returns $wp_query only with posts which have files of certain types attached with podPress
	* This function helps to create e.g. feeds with posts which have m4a files attached.
	*
	* @package podPress
	* @since 8.8.5 beta 3
	*
	* @param Array $wp_query - GLOBAL $wp_query
	* @param Array $allowed_extensions - an array of string e.g. Array('m4a', 'm4v')
	*
	* @return Array $wp_query
	*/
	function podpress_only_posts_with_certain_files($input, $allowed_extensions = array()) {
		if (is_array($allowed_extensions) AND False == empty($allowed_extensions)) {
			// filter all the posts which have an torrent file attached with podPress
			$unfiltered_posts = $input->posts;
			$posts = Array();
			$post_count = 0;
			foreach ($unfiltered_posts as $post) {
				$is_a_torrentpost = FALSE; 
				if ( is_array($post->podPressMedia) ) {
					foreach ($post->podPressMedia as $podPressMedia) {
						foreach ($allowed_extensions as $ext) {
							$extlen = (strlen($ext)+1);
							if ( $extlen < strlen($podPressMedia['URI']) AND ('.'.$ext == strtolower(substr($podPressMedia['URI'], -$extlen))) ) {
								$is_a_torrentpost = TRUE;
								break;
							}
						}
					}
				}
				if (TRUE === $is_a_torrentpost) {
					$posts[] = $post;
					$post_count++;
				}
			}
			$input->post_count = $post_count;
			$input->posts = $posts;
		}
		return $input;
	}
