...
*/
	...
	function podPress_maybe_add_column($table_name, $column_name, $create_ddl) {
		GLOBAL $wpdb, $debug;
		//~ printphpnotices_var_dump('table name: "'.$table_name.'"');
		//~ printphpnotices_var_dump('column name: "'.$column_name.'"');
		//~ printphpnotices_var_dump('query string: '.$create_ddl);
		foreach ( $wpdb->get_col("DESC $table_name", 0) as $column ) {
			if ( $debug ) { printf(__('checking %1$s == %2$s<br />', 'podpress'), $column, $column_name); }
			if ( $column == $column_name ) {
				//~ printphpnotices_var_dump('the "'.$column_name.'" column exists');
				return TRUE;
			}
		}
		// didn't find it try to create it.
		...
		foreach ( $wpdb->get_col("DESC $table_name", 0) as $column ) {
			if ( $column == $column_name ) {
				//~ printphpnotices_var_dump('adding the "'.$column_name.'" column');
				return TRUE;
			}
		}
		//~ printphpnotices_var_dump('the "'.$column_name.'" column may not exist and was probably not added');
		return FALSE;
	}
	...
	function podPress_showID3tags($mediafile) {
		...
		if(!is_array($fileinfo) OR FALSE == isset($fileinfo['comments']) ) {
			return '<div class="updated message">'.__('The file has probably no information like artist, genre, album, etc.', 'podpress').'<br /><pre>'.var_export($fileinfo, true).'</pre></div>';
		}
