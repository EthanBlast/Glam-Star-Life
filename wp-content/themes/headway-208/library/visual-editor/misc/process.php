...
switch($_GET['process']){
	...
	case 'options-js':
		do_action('headway_custom_leaf_options_js_'.$_POST['leaf'], $_POST['id']);
	break;
	
	
	
	case 'leaf-options':
		do_action('headway_custom_leaf_'.$_POST['leaf'].'_options', $_POST['id'], false);
	break;
	
	
	
	case 'get-leaf-options-width':
		do_action('headway_custom_leaf_'.$_POST['leaf'].'_options', $_POST['id'], true);
	break;
	
	
	
	case 'nav-item-options':
		headway_navigation_item_options($_POST['nav-item'], urldecode($_POST['nav-item-name']));
	break;
	...
	case 'proxy':
		...
		if(function_exists('curl_init')){
			...
			if($_POST['use_auth']) curl_setopt($ch, CURLOPT_USERPWD, $username.':'.$password);
	...
	case 'import-style':
		echo $_GET['callback'] . '(' . headway_import_style(array('file' => $_POST['path'])) . ')';
	break;
	
	
	
	case 'load-style':
		$style = headway_get_option('style-'.urldecode($_POST['style-name']));
	...
	case 'build-color-scheme':	
		$style = headway_decode_array(file_get_contents(HEADWAYLIBRARY.'/installation/styles/Magazine.hwstyle'));
		
		$primary = headway_hex_to_rgb($_POST['primary-color']);
		$secondary = headway_hex_to_rgb($_POST['secondary-color']);
	...
	case 'delete-style':
		$styles = headway_get_option('styles');
		$style_id = str_replace('style-', '', $_POST['style-id']);
		
		unset($styles[$_POST['style-name'].'-'.$style_id]);
		
		headway_update_option('styles', $styles);
		
		headway_delete_option('style-'.$_POST['style-name'].'-'.$style_id);
	break;
	...
	case 'rename-style':
		...
		$styles[$_POST['style-name'].'-'.$style_id]['style-name'] = $_POST['style-new-name'];
		
		//Replace the style in the styles array with the new renamed style.
		$styles[$_POST['style-new-name'].'-'.$style_id] = $styles[$_POST['style-name'].'-'.$style_id];
		...
		$style_content = headway_decode_array(headway_get_option('style-'.$_POST['style-name'].'-'.$style_id));
		...
		if(isset($style_content['style-name'])){
			$style_content['style-name'] = $_POST['style-new-name'];
		}
		
		//Save new style content to DB
		headway_update_option('style-'.$_POST['style-new-name'].'-'.$style_id, maybe_serialize($style_content));
	break;
	...
	case 'visual-editor-run-up':
		...
		$headway_settings['headway-css-url'] = home_url().'/?headway-trigger=global-css&'.headway_get_option('css-last-updated').'&visual-editor-open=true';
		...
		$headway_settings['fonts'] = $headway_fonts;
		
		global $headway_font_families;
		$headway_settings['font-families'] = $headway_font_families;
		////////////
		
		//Leaf Sizing
		$leafs = headway_get_page_leafs($_POST['page-id']);
	...
	case 'edit-leaf':
		...
		$array_keys = array_keys($_POST['leaf-options']);
		$leaf_id = $array_keys[0];
		
		$options = $_POST['leaf-options'][$leaf_id];
		$config = $_POST['config'][$leaf_id];
		...
		//Leaf Config
		$new_config = headway_get_leaf($leaf_id);
		$new_config = $new_config['config'];
		
		$new_config['show-title'] = $config['show-title'];
		$new_config['title-link'] = $config['leaf-title-link'];
		$new_config['custom-classes'] = $config['custom-css-classes'];
		
		//Finish Up
		headway_update_leaf($leaf_id, array('config' => $new_config, 'options' => $options));
	break;
	
	
	
	case 'get-leaf-content':
		headway_build_leaf_content($_POST['leaf-id'], true);
	break;
	...
	case 'export-leaf-template':
		...
		echo headway_json_encode(headway_get_option('leaf-template-'.urldecode($_GET['template-id'])));
	break;
	...
	case 'save-leaf-template':
		...
		$template['id'] = $random;
		$template['name'] = urldecode($_POST['template-name']);
		...
		$template['leafs'] = headway_get_page_leafs($_POST['page']);
		$template['columns'] = $_POST['columns'];
		...
		if($_POST['column-1-width'] != 'unserialized') $template['column-widths'][1] = $_POST['column-1-width'];
		if($_POST['column-2-width'] != 'unserialized') $template['column-widths'][2] = $_POST['column-2-width'];
		if($_POST['column-3-width'] != 'unserialized') $template['column-widths'][3] = $_POST['column-3-width'];
		if($_POST['column-4-width'] != 'unserialized') $template['column-widths'][4] = $_POST['column-4-width'];
				
		headway_update_option('leaf-template-'.$template['name'].'-'.$template['id'], maybe_serialize($template));
	...
	case 'rename-template':
		//Fetch Templates
		$templates = headway_get_option('leaf-templates'); 
		$template_id = str_replace('template-', '', $_POST['id']);
				
		//Replace template name in template options array to the new name
		$templates[$_POST['template-name'].'-'.$template_id]['name'] = $_POST['new-name'];
		
		//Replace the template in the templates array with the new renamed template.
		$templates[$_POST['new-name'].'-'.$template_id] = $templates[$_POST['template-name'].'-'.$template_id];
		
		//Delete old template
		unset($templates[$_POST['template-name'].'-'.$template_id]);
		
		//Save templates to DB
		headway_update_option('leaf-templates', $templates);
		
		//Fetch actual template content and decode it
		$template_content = headway_decode_array(headway_get_option('leaf-template-'.$_POST['template-name'].'-'.$template_id));
		
		//Delete old template content
		headway_delete_option('template-'.$_POST['template-name'].'-'.$template_id);
		
		//Save new template content to DB
		headway_update_option('template-'.$_POST['new-name'].'-'.$template_id, maybe_serialize($template_content));
	break;
	...
	case 'delete-template':
		...
		$template_id = str_replace('template-', '', $_POST['template-id']);
	...
	case 'load-template':
		...
		$template = headway_decode_array(headway_get_option('leaf-template-'.html_entity_decode(urldecode($_POST['template-name']))));
	...
	case 'set-default-leafs-template':
		return headway_update_option('default-leafs-template', array('name' => urldecode($_POST['template-name']), 'id' => str_replace('template-', '', urldecode($_POST['id']))));
	break;
	...
	case 'analyze-image':
		$colors = headway_image_color_palette(headway_upload_path(true).'/header-uploads/'.$_POST['image'], 10, 5, true);
