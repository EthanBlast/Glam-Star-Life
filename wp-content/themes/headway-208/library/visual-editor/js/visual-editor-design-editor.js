...
function headway_design_editor() {
	...
	//Bind Visual Font
	documentBlurFunction = function(e){
		if(!hwjs(e.target).is('div.font-family-select-options-open, div.font-family-select-options-open *, span.font-family-select-button')){
			hwjs('div.font-family-select-options-open').removeClass('font-family-select-options-open');
			
			hwjs(document).unbind('click', documentBlurFunction);
		}
	}
	
	visual_font_selector_button_action = function($$, selector, select_input){		
		$$.addClass('select-button-options-open');

		options = $$.siblings('div.' + selector);
		
		options.addClass('font-family-select-options-open'); //Add the class for delegation and to not make a blur on every single one

		options.scrollTop(0); //Reset the scroll		
		options.scrollTop(options.children('.selected').position().top);

		hwjs(document).bind('click', documentBlurFunction);
	}
	
	visual_font_selector_option_action = function($$, selector, select_input, action){
		if(typeof action == 'undefined' || !action) action = false;

		$$.parent().siblings('select' + select_input).val($$.attr('class').replace('selected', '').replace(' ', '')).trigger('change');
		
		if(action){
			options = $$.parents('div.font-family-select-options');
			
			$$.addClass('selected').siblings('.selected').removeClass('selected');
			
			options.siblings('span.font-family-select-button').removeClass('select-button-options-open').text($$.text()).css('fontFamily', $$.css('fontFamily'));

			options.removeClass('font-family-select-options-open');
		}
	}
	
	bind_visual_font_selector = function(selector, select_input){		
		hwjs('span.' + selector).bind('click', function(){
			visual_font_selector_button_action(hwjs(this), selector, select_input, true);
		});
		
		hwjs('div.' + selector).delegate('span', 'click', function(){
			visual_font_selector_option_action(hwjs(this), selector, select_input, true);
		});
	}
	
	//Normal Fonts
	hwjs('div#visual-editor-sidebar').delegate('span.font-family-select-button', 'click', function(){
		visual_font_selector_button_action(hwjs(this), 'font-family-select-options', '.font-family');
	});
	
	hwjs('div#visual-editor-sidebar div.font-family-select-options').delegate('span', 'click', function(){
		visual_font_selector_option_action(hwjs(this), 'font-family-select-options', '.font-family');
	});
	
	//Mass Fonts
	bind_visual_font_selector('mass-font-select-titles', '#mass-font-select-titles');
	bind_visual_font_selector('mass-font-select-content', '#mass-font-select-content');
	
	//Wizard Fonts
	bind_visual_font_selector('wizard-fonts-titles', '#wizard-fonts-titles');
	bind_visual_font_selector('wizard-fonts-content', '#wizard-fonts-content');
	...
	hwjs('div#de-tab').delegate('select.font-select', 'change', function(){	
		...
		if(property == 'font-size'){
			...
			value = headway_settings['fonts'][hwjs(this).val()]['family'];
			
			options = hwjs(this).siblings('div.font-family-select-options');
			selected_option = options.children('.' + hwjs(this).val())
			
			selected_option.addClass('selected').siblings('.selected').removeClass('selected');
			
			options.siblings('span.font-family-select-button').removeClass('select-button-options-open').text(selected_option.text()).css('fontFamily', selected_option.css('fontFamily'));
	...
	hwjs('select#mass-font-select-titles').change(function(){
		...
		hwjs('select.title-fonts:not(.mass-font-change)').val(mass_value).trigger('change');
	...
	hwjs('select#mass-font-select-content').change(function(){
		...
		hwjs('select.font-family:not(.mass-font-change, .title-fonts)').val(mass_value).trigger('change');
	...
	hwjs('a#save-style-submit').click(function(){
		...
		hwjs.post( 
			headway_blog_url+'/?headway-trigger=process&process=save-style&style-name=' + escape(style_name) + '&color-primary=' + escape(color_primary) + '&color-secondary=' + escape(color_secondary) + '&color-tertiary=' + escape(color_tertiary), 
			design_data,
			function(response){
				style_id = response;
	...
	var import_style = new qq.FileUploader({
        ...
        action: headway_blog_url + '/?headway-trigger=process&process=ve-upload&what=style',
	    ...
	    onComplete: function(id, fileName, response){
			...
			hwjs.ajax({
			  type: 'POST',
			  data: {'path':fileName},
			  url: headway_blog_url+'/?headway-trigger=process&process=import-style&callback=?',
			  dataType: 'json',
	...
	hwjs('a#load-style').click(function(){
		...
		hwjs('div#ve-working-overlay').show().animate({'opacity':1}, 250, false, function(){
			hwjs.ajax({
			  type: 'POST',
			  data: {'style-name':escape(style_name + '-' + style_id)},
			  url: headway_blog_url+'/?headway-trigger=process&process=load-style&callback=?',
			  dataType: 'json',
			  success: function(data){
					...
					for (var i=0;i<style.length;i++){
						...
						if(hwjs('#'+style[i].property_type+'-'+style[i].element+'-'+style[i].property).length > 0){
							...
							if(style[i].property == 'font-family' && style[i].value.indexOf(',') >= 0){		
								if(value == 'trebuchet ms, sans-serif' || value == 'tahoma, sans-serif'){
									value = 'helvetica, sans-serif';
								}
																						
								value = headway_settings['font-families'][value]['id'];
	...
	hwjs('a#save-style-settings-button').click(function(){
		...
		hwjs.ajax({
			type: 'POST',
			data: {'style-id': style_id, 'style-name': style_name, 'style-new-name': rename_input.val()},
			url: headway_blog_url+'/?headway-trigger=process&process=rename-style',
			async: false,
			success: function(data){	
	...
	hwjs('a#delete-style-button').click(function(){
		...
		if(confirm('Are you sure?') === true){			
			hwjs.ajax({
				type: 'POST',
				data: {'style-id': style_id, 'style-name': style_name},
				url: headway_blog_url+'/?headway-trigger=process&process=delete-style',
				async: false,
				success: function(data){	
