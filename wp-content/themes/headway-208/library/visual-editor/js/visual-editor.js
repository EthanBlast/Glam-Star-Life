...
function headway_initiate_sidebar_scroll(){
	hwjs('#visual-editor-sidebar-content').css('overflow-y', 'auto').scrollbarPaper();
}
...
function headway_save_editor(redirect){	
	...
	hwjs.post( headway_blog_url + '/?headway-trigger=visual-editor-action', { encoded: headway_encode64(form_data), 'headway-ve-nonce':nonce }, function(){ 
		
		hwjs.post( headway_blog_url + '/?headway-trigger=visual-editor-action', { encoded: headway_encode64(color_data), 'headway-ve-nonce':nonce, 'truncate':'color' }, function(){
		
			hwjs.post( headway_blog_url + '/?headway-trigger=visual-editor-action', { encoded: headway_encode64(font_data), 'headway-ve-nonce':nonce, 'truncate':'font' }, function(){
			
				hwjs.post( headway_blog_url + '/?headway-trigger=visual-editor-action', { encoded: headway_encode64(border_data), 'headway-ve-nonce':nonce, 'truncate':'sizing' }, function(){
...
function headway_ready_help_links(link){
	...
	hwjs.ajax({
	  type: 'POST',
	  data: {'use_auth': 'true', 'url': escape('http://headwaythemes.com/resources/inline-documentation/?get-slug-id='+link.attr('href').replace('http://headwaythemes.com/', ''))},
	  url: headway_blog_url+'/?headway-trigger=process&process=proxy',
	  cache: false,
	  success: function(id){
    ...
    hwjs('div#help-box-content').load(headway_blog_url+'/?headway-trigger=process&process=proxy', {'use_auth': 'true', 'url':escape('http://headwaythemes.com/resources/inline-documentation/?slug='+link.attr('href').replace('http://headwaythemes.com/', ''))}, function(){ 
...
function headway_ready_help_selector(selector){
	hwjs('div#help-box-content').html('<p class="loading"><img src="' + headway_settings['template-directory'] + '/library/shared-media/images/loading.gif" class="loading-image" /></p>');
	hwjs('div#help-box-content').load(headway_blog_url+'/?headway-trigger=process&process=proxy', {'use_auth': 'true', 'url': escape('http://headwaythemes.com/resources/inline-documentation/?id='+selector.val())}, function(){
		hwjs('div#help-box-content a[href*="headwaythemes.com/documentation"]').click(
		    function(){
		        headway_ready_help_links(hwjs(this));
...
function headway_set_up(){
	...
	hwjs('div#visual-editor-menu a:not(.no-link)').click(function(){
		if(hwjs(this).hasClass('use-overlay')){
			...
			if(!hwjs('div#help-box-bar-left').hasClass('loaded')){
				hwjs('div#help-box-bar-left').load(headway_blog_url+'/?headway-trigger=process&process=proxy', {'use_auth': 'true', 'url': escape('http://headwaythemes.com/resources/inline-documentation/?dropdown=true')}, function(){
	...
	hwjs('a#visual-editor-sidebar-toggle').click(function(){
		if(hwjs('div#visual-editor-sidebar').hasClass('collapsed')){
			hwjs('div#headway-visual-editor').animate({marginLeft: 323}, 750);
			...
			hwjs('div#visual-editor-sidebar').animate({left: -323}, 750);
	...
	hwjs('div.floaty-box-close h4.floaty-box-header a.close').click(function(){
		...
		if(box.attr('id') == 'wizard-box'){
			...
			hwjs.ajax({url: headway_blog_url+'/?headway-trigger=process&process=skip-wizard'});
		} else {
	...
	hwjs('.ve-tooltip').tooltip({track: true, delay: 0, showURL: false, fade: 250, positionLeft: false, id: 've-tooltip-bubble'});
...
hwjs(document).ready(function(){	
	hwjs.ajax({
		type: 'POST',
		data: {'page-id': hwjs('input#current-page').val()},
		url: headway_blog_url+'/?headway-trigger=process&process=visual-editor-run-up&callback=?',
		async: false,
		dataType: 'json',
