...
function headway_visual_editor_options(){
	...
	/* Header */
		...
		if(hwjs('div#header-image').length > 0){
			var header_uploader = new qq.FileUploader({
		        ...
		        action: headway_blog_url + '/?headway-trigger=process&process=ve-upload&what=header',
			    ...
			    onComplete: function(id, fileName, response){
					...
					if(hwjs('input#enable-header-resizing').attr('checked')){
						image_url = headway_settings['url'] + '/?headway-trigger=thumbnail&src=' + headway_settings['upload-path'] + '/header-uploads/' + escape(response.filename) + '&w=' + headway_settings['wrapper-width'];
					} else {
			...
			hwjs('input#enable-header-resizing').click(function(){
				...
				if(current_header_image.match('http')){
					...
					if(hwjs(this).is(':checked')){
						...
						thumbnail = headway_settings['url'] + '/?headway-trigger=thumbnail&src=' + escape(source) + '&q=90&w=' + hwjs('div#header').width() + 'px&zc=1';
	...
	/* Body */
		if(hwjs('div#body-background-image').length > 0){
			var background_uploader = new qq.FileUploader({
		        ...
		        action: headway_blog_url + '/?headway-trigger=process&process=ve-upload&what=background',
				allowedExtensions: ['jpg', 'jpeg', 'gif', 'png'],
			    onComplete: function(id, fileName, response){
	...
	/* Navigation */
		...
		function load_navigation_item_options(nav_id, nav_name){
			...
			hwjs('div#navigation-control-' + nav_id).load(headway_blog_url+'/?headway-trigger=process&process=nav-item-options', {'nav-item': nav_id, 'nav-item-name': nav_name}, function(){
			...
			var wizard_header_uploader = new qq.FileUploader({
			    ...
			    onComplete: function(id, fileName, response){
					...
					hwjs.ajax({
					  ...
					  url: headway_blog_url+'/?headway-trigger=process&process=analyze-image&callback=?',
					  dataType: 'json',
			...
			hwjs('a.wizard-finish').click(function(){	
				...
				hwjs('div#ve-working-overlay').show().animate({'opacity':1}, 250, false, function(){
					if(primary_color != 'ffffff' && secondary_color != 'ffffff' && tertiary_color != 'ffffff'){
						hwjs.ajax({
							type: 'POST',
							data: {'primary-color': primary_color, 'secondary-color': secondary_color},
							url: headway_blog_url+'/?headway-trigger=process&process=build-color-scheme&callback=?',
							dataType: 'json',
							success: function(data){								
								style = data['style'];
								colors = data['colors'];
	...
	/* Start (Importing) */
		var import_leaf_template = new qq.FileUploader({
	        ...
	        action: headway_blog_url + '/?headway-trigger=process&process=ve-upload&what=leaf-template',
		    ...
		    onComplete: function(id, fileName, response){
				...
				hwjs.ajax({
				  type: 'POST',
				  data: {'path': fileName},
				  url: headway_blog_url+'/?headway-trigger=process&process=import-leaf-template&callback=?',
				  dataType: 'json',
				  success: function(data){				
						name = data['name'];
						id = data['id'];
