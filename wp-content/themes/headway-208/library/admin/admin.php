...
function headway_add_menus(){
	...
	if(defined('HEADWAYHIDEMENUS') && HEADWAYHIDEMENUS === true || !headway_can_user('headway_admin')) return false;
		
	$headway_menu_name = (headway_get_option('hide-admin-menu-version')) ? 'Headway' : 'Headway '.HEADWAYVERSION;
	
	$menu[57] = array('', 'read', 'separator-headway', '', 'wp-menu-separator');
	
	$headway_menu = add_menu_page('Headway Configuration', $headway_menu_name, 'manage_options', 'headway', 'headway_configuration', HEADWAYURL.'/library/admin/images/headway_16.png', 58); 
	add_action("admin_print_scripts-$headway_menu", 'headway_admin_panel_head');

	$configuration_page = add_submenu_page('headway', 'Configuration', 'Configuration', 'manage_options', 'headway', 'headway_configuration');
	...
	if(headway_get_option('active-skin', true, true) && headway_get_option('active-skin', true, true) != 'none'){
		$skins_options_page = add_submenu_page('headway', $headway_skin_name.' Options', $headway_skin_name.' Options', 'manage_options', 'headway-skin-options', 'headway_skin_options');
		add_action("admin_print_scripts-$skins_options_page", 'headway_admin_panel_head');
	}
		
	if((is_main_site() || (is_multisite() && !get_site_option('disable-tools'))) && headway_can_user('headway_tools')){
		$tools_page = add_submenu_page('headway', 'Tools', 'Tools', 'manage_options', 'headway-tools', 'headway_tools');
		add_action("admin_print_scripts-$tools_page", 'headway_admin_panel_head');
	}
	
	if((is_main_site() || (is_multisite() && !get_site_option('disable-easy-hooks'))) && headway_can_user('headway_easy_hooks')){
		$hooks_page = add_submenu_page('headway', 'Easy Hooks', 'Easy Hooks', 'manage_options', 'headway-easy-hooks', 'headway_easy_hooks');
		add_action("admin_print_scripts-$hooks_page", 'headway_easy_hooks_head');
	}

	if(headway_can_user('headway_visual_editor')){
		$visual_editor_page = add_submenu_page('headway', 'Visual Editor', 'Visual Editor', 'manage_options', 'headway-visual-editor', 'headway_visual_editor_forward' );
...
function headway_easy_hooks_head(){	
	...
	wp_enqueue_script('headway_admin_js', get_bloginfo('template_directory').'/library/admin/js/admin-easy-hooks.js', array('jquery', 'jquery-ui-tabs'));
}
...
function headway_visual_editor_forward(){
	headway_admin_header('Headway Visual Editor');
		echo 'You are now being redirected.  If you are not redirected within 5 seconds, click <a href="'.get_bloginfo('siteurl').'/?visual-editor=true"><strong>here</strong></a>.';
		echo '<meta http-equiv="refresh" content="0;URL='.get_bloginfo('siteurl').'/?visual-editor=true">';
	headway_admin_footer();
}
...
function headway_update_notice(){
	if(!current_user_can('manage_options') || headway_get_option('disable-version-checks')) return false;
