...
?>
	...
	<div id="seo-preview">
		...
		<p id="seo-preview-bottom"><span id="seo-preview-url"><?php echo str_replace('http://', '', home_url()) ?></span> - <span>Cached</span> - <span>Similar</span></p>
	</div>
			...
			$id = $input_options['id'];
				...
				if($input_options['type'] == 'text'){
					...
					$options .= '<tr class="label"><th valign="top" scope="row"><label for="'.$id.'">'.$input_options['name'].'</label></th></tr>
								 <tr><td><input type="text" style="width: 95%;" value="'.htmlspecialchars(stripslashes($data[$input_options['id']])).'" size="50" id="'.$id.'" name="'.$this->id.'['.$input_options['id'].']"/></td></tr>';
					...
					$options .= '<tr class="label"><th valign="top" scope="row"><label for="'.$id.'">'.$input_options['name'].'</label></th></tr>
								<tr><td><textarea style="width: 95%;" rows="6" cols="50" id="'.$id.'" name="'.$this->id.'['.$input_options['id'].']">'.htmlspecialchars(stripslashes($data[$input_options['id']])).'</textarea></td></tr>';
