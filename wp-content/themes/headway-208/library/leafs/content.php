...
class content_leaf {
	...
	function set_options(){
		...
		$this->featured_posts = (isset($this->leaf['options']['featured-posts'])) ? $this->leaf['options']['featured-posts'] : headway_get_option('featured-posts');
	}
	...
	function display_featured_post($args = array()){
		...
		if(!isset($this->leaf['options']['hide-content']) || !$this->leaf['options']['hide-content']){
			...
			echo "\n".'<div id="post-'.get_the_id().'" class="'.headway_post_class(false).'">'."\n";
		...
		if($single || $this->leaf['options']['force-comment-visibility']){
			if(!isset($this->leaf['options']['hide-content']) || !$this->leaf['options']['hide-content']){
	...
	function display_post($args = array()){
		...
		echo "\n".'<div id="post-'.get_the_id().'" class="'.headway_post_class(false).' small-post'.$small_excerpts_class.'">'."\n";
		...
		if($this->leaf['options']['force-comment-visibility'] && !$small_excerpts){
			do_action('headway_before_comments', 'post');
			$wp_query->is_single = true;
			comments_template();
			$wp_query->is_single = $single;
	...
	function display_more_link($args = array()){	
		...
		if(strpos($post->post_content, '<!--more-->') || (isset($excerpts) && $excerpts)){
			echo '<p class="more-link-container"><a href="'.get_permalink().'" class="more-link">'.headway_get_option('read-more-text').'</a></p>';
		}
	}
...
?>
	<ul class="clearfix tabs">
        ...
        <li><a href="#comments-options-tab-<?php echo $leaf['id'] ?>">Comments</a></li>
	...
	<div id="comments-options-tab-<?php echo $leaf['id'] ?>">
		<table class="tab-options" id="leaf-options-<?php echo $leaf['id'] ?>-comments">
			...
			<tr class="no-border">
				<th scope="row"><label>Comment Visibility</label></th>
				<td>
					<p class="radio-container">
						<input type="checkbox" class="radio headway-visual-editor-input" id="<?php echo $leaf['id'] ?>_force_comment_visibility" name="leaf-options[<?php echo $leaf['id'] ?>][force-comment-visibility]"<?php echo headway_checkbox_value($leaf['options']['force-comment-visibility']) ?> />
						<label for="<?php echo $leaf['id'] ?>_force_comment_visibility">Allow Comments to Display On Non-single Post Page</label>
					</p>
				</td>	
			</tr>
		</table>
