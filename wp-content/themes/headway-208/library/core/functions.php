...
function headway_gzip(){
	if(headway_get_option('gzip') == 1 && !headway_is_plugin_caching() && !class_exists('All_in_One_SEO_Pack') && !isset($_GET['duid'])) if ( extension_loaded('zlib') ) ob_start('ob_gzhandler');
}
...
function headway_current_url(){
	...
	if ($_SERVER["SERVER_PORT"] != "80") {
		$url .= $http_host.":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
	} else {
...
function headway_current_page($get_real_page = false){
	...
	if(!isset($_GET['visual-editor-system-page']) || (isset($_GET['visual-editor-system-page']) && is_numeric($_GET['visual-editor-system-page']))){
		...
		if(is_tax()){
			...
			$system_page = 'custom-archive-'.$queried_object->taxonomy;
		}
...
function headway_convert_hr_to_bytes($size){
...
function headway_convert_bytes_to_hr($bytes) {	
...
function headway_upload_max(){	
	if ( is_multisite() ) {
		require_once(ABSPATH . 'wp-admin/includes/ms.php');
	}
	
	$u_bytes = headway_convert_hr_to_bytes(ini_get('upload_max_filesize'));
	$p_bytes = headway_convert_hr_to_bytes(ini_get('post_max_size'));	
...
/**
 * Converts a JSON or serialized string back to an array.
 * 
 * @param string $string JSON or serialized string to be converted.
 *
 * @return array
 **/
function headway_decode_array($string){
...
function headway_can_visually_edit(){
	if(headway_can_user('headway_visual_editor') && (current_user_can('manage_options') || is_super_admin())){
		return true;
	} else {
		return false;
	}
...
function headway_generate_cache($what = false){
	...
	if(in_array('headway', $what)){
		//Fetch CSS from DB
		$headway_css = headway_generate_global_css();	
	...
	if(in_array('leafs', $what)){
		$leafs_css = headway_generate_leafs_css();
	...
	if(in_array('scripts', $what)){
		$scripts_js = headway_generate_scripts();
...
function headway_print_debug_output(){
	...
	if(defined('SAVEQUERIES') && SAVEQUERIES === true && (defined('WP_DEBUG') && WP_DEBUG === true) || (defined('HEADWAY_DEBUG') && HEADWAY_DEBUG === true)){
		global $wpdb; 
...
function headway_clean_style($style_path, $elements_to_remove){
	$styles = headway_decode_array(file_get_contents($style_path));
