...
class HeadwaySkin {
	...
	function exec(){
		...
		} elseif(maybe_serialize(headway_get_option('skin-options')) != maybe_serialize($this->options)){
	...
	function create_text_input($options){
		...
		$return .= '<th scope="row"><label for="skin_option_'.str_replace('-', '_', $options['name']).'">'.$options['label'].'</label></th>
						<td><input type="text" name="skin-options['.$options['name'].']" id="skin_option_'.str_replace('-', '_', $options['name']).'" value="'.stripslashes(htmlspecialchars(headway_skin_option($options['name'], true))).'" />';
	...
	function create_textarea($options){
		...
		$return .= '<th scope="row"><label for="skin_option_'.str_replace('-', '_', $options['name']).'">'.$options['label'].'</label></th>
						<td><textarea class="regular-text" name="skin-options['.$options['name'].']" id="skin_option_'.str_replace('-', '_', $options['name']).'" rows="6">'.stripslashes(htmlspecialchars(headway_skin_option($options['name'], true))).'</textarea>';
...
class HeadwayLeaf {	
	...
	function options($leaf_id, $get_width = false){
		if(!$get_width){
			...
			echo '<h4 class="floaty-box-header">'.$this->name.' <span class="leaf-id">#'.$id.'</span></h4>
			<div class="tabs">';
...
class HeadwayLeafsHelper {
	...
	function create_text_input($options, $leaf_id, $type = 'leaf-options'){
		...
		echo '<th scope="row"><label for="'.$leaf_id.'_'.str_replace('-', '_', $options['name']).'">'.$options['label'].'</label></th>
						<td><input type="text" class="headway-visual-editor-input" name="'.$type.'['.$leaf_id.']['.$options['name'].']" id="'.$leaf_id.'_'.str_replace('-', '_', $options['name']).'" value="'.stripslashes(htmlspecialchars($options['value'])).'" /></td>';
	...
	function create_textarea($options, $leaf_id, $type = 'leaf-options'){
		...
		echo '<tr class="no-border">
				...
				<tr class="textarea'.$no_border.'">
					<td colspan="2"><textarea class="text-content headway-visual-editor-input" name="'.$type.'['.$leaf_id.']['.$options['name'].']" id="'.$leaf_id.'_'.str_replace('-', '_', $options['name']).'">'.stripslashes(htmlspecialchars($options['value'])).'</textarea></td>
			  </tr>';
