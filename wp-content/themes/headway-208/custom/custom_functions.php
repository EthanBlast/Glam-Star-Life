...
//the_post_thumbnail(); 
//add_theme_support( 'post-thumbnails' );
//set_post_thumbnail_size( 230, 230, true ); // Normal post thumbnails
//add_image_size( 'homepage-post-thumbnail', 460, 920 ); // Permalink thumbnail size
//the_post_thumbnail( 'homepage-post-thumbnail' ); 
	...
	// while ($wp_query->have_posts()) : $wp_query->the_posts();
	  //   print '<h2> <a href='.the_permalink().'>'.the_title().'</a></h2>'.the_excerpt();
     //endwhile;
	 //print '<div><div>'.previous_posts_link('&laquo; Previous').'</div><div>'.next_posts_link('More &raquo;').'</div></div>';
	 //$wp_query=null;
	 //$wp_query = $temp;
	}
	else{
