...
function headway_show_header($item = false){
		...
		if(headway_get_option('show-header-rss-link')){
			echo '<a id="header-rss-link" href="'.headway_rss().'">'.__('Subscribe via RSS', 'headway').'</a>';
		} elseif(headway_visual_editor_open()){
			echo '<a id="header-rss-link" href="'.headway_rss().'" style="display: none;">'.__('Subscribe via RSS', 'headway').'</a>';
		}
		...
		//If exists, is uploaded, and resizing ENABLED.
			...
			$header_link_content = '<img src="'.headway_upload_url().'/header-uploads/'.headway_get_option('header-image').'" alt="'.get_bloginfo('name').'" />'."\n";
		...
		//Else no image.
			...
			if(headway_get_skin_option('show-tagline') && !is_front_page()){
				echo '<span id="tagline">'.get_bloginfo('description').'</span>'."\n";
			} elseif(headway_get_skin_option('show-tagline') && is_front_page()){
...
function headway_breadcrumbs($item = false, $echo = false, $force_display = false){	
	if(headway_breadcrumbs_test() || $force_display){
		...
		$return = '<div id="breadcrumbs"'.$rearrange_item.'><p>'.__('You Are Here:', 'headway').' &nbsp; <a href="'.get_bloginfo('url').'">'.__('Home', 'headway').'</a>';
