...
function headway_enqueue_scripts(){	
	...
	if(count($load_scripts) > 0 || isset($headway_custom_leaf_js_active)){
		...
		if(headway_cache_exists('scripts.js') && !headway_visual_editor_open() && !$cleared_cache && headway_allow_caching())
			...
			wp_enqueue_script('headway_js_settings', get_bloginfo('url').'/?headway-js&amp;'.$last_updated, array('jquery'));
	}
...
function headway_print_stylesheets(){	
	...
	if(headway_get_option('print-css')) echo '<link rel="stylesheet" type="text/css" href="'.get_bloginfo('template_directory').'/media/css/print.css" media="print" /><!-- Headway Print Styles -->'."\n";
...
function headway_primary_stylesheets(){
	...
	if(headway_cache_exists('headway.css') && !headway_visual_editor_open() && !$cleared_cache && headway_allow_caching()){
		...
		echo '<link rel="stylesheet" type="text/css" href="'.get_bloginfo('url').'/?headway-css=&amp;'.$last_updated.$skin_preview.$visual_editor_open.'"'.$css_media.' /><!-- Headway Elements -->'."\n";
	}	
	...
	if(headway_cache_exists('leafs.css') && !headway_visual_editor_open() && !$cleared_cache && headway_allow_caching()){	
		...
		echo '<link rel="stylesheet" type="text/css" href="'.get_bloginfo('url').'/?headway-leafs-css=&amp;'.$last_updated.'"'.$css_media.' /><!-- Headway Leaf Sizing -->'."\n";
	}
}
