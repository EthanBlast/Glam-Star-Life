					...
					if((isset($element[3]) && $element[3]) || (isset($element[4]) && $element[4])){
						...
						if(isset($element[3]) && $element[3]){
							...
							if(!is_array($element[3]) || (is_array($element[3]) && in_array('font-family', $element[3]))){
								...
								$return .= '<tr class="alt">
									<th scope="row"><label for="fonts-'.$nice_id.'-font-family">'.__('Font/Typeface', 'headway').'</label></th>
									<td>
										<select id="font-'.$nice_id.'-font-family" name="fonts['.$nice_id.'][font-family]" class="font-select font-family headway-visual-editor-font-input'.$title_fonts.'" selector="'.$selector.'">
						...
						if(isset($element[4]) && $element[4]){	
							$advanced_font_property_row_count = (isset($element[3]) && $element[3]) ? 0 : -1;
							...
							if(!is_array($element[4]) || (is_array($element[4]) && in_array('styling', $element[4]))){
								$advanced_font_property_row_count++;
								
								$alt = $advanced_font_property_row_count%2 ? null : ' alt';
