function ready_leafs(leaf){
	...
	hwjs(leaf_nonobject).children('.headway-leaf-inside').prepend('<div class="leaf-icon-container"><a href="" title="This is the ID of the leaf, which is used for duplicate sidebars and advanced coding." class="tooltip leaf-id"></a><div class="leaf-icon"></div></div>');
	hwjs(leaf_nonobject).children('.headway-leaf-inside').prepend('<div class="leaf-control"><a class="leaf-height leaf-height-fluid tooltip" href="" title="Toggle fluid height of leaf.">Fluid Height</a><a class="leaf-alignment-toggle tooltip" href="" title="Toggle align to left, right, or tell the leaf to start a new row.">Align/New Row</a><a class="edit tooltip" href="" title="Open the options for this leaf.">Edit</a><a class="delete tooltip" href="" title="Delete this leaf.">Delete</a></div><a href="#" class="precise-resize">Precise Resize</a>');
	...
	hwjs(leaf_nonobject).each(function(){
		...
		url = headway_blog_url+'/?headway-process=leaf-sizes&callback=?&page-id='+hwjs('input#current-page').val();
		
		hwjs('div#'+leaf_full_id+' .ve-tooltip').tooltip({track: true, delay: 0, showURL: false, fade: 250,extraClass: "ve-leaf-tooltip"});
		...
		if(leaf_inside.children('.leaf-top').length > 0){
			leaf_inside.children('.leaf-top').attr('title', 'Double-click to edit title.').addClass('ve-tooltip').tooltip({track: true, delay: 0, showURL: false, fade: 250, positionLeft: false, extraClass: 've-tooltip-default'});
...
function prepare_leaf_options(leaf_id, leaf_options_width){
	...
	hwjs('div#control-'+leaf_id+' .headway-save-leaf-button').click(function(){
		...
		leaf_data = hwjs('div#control-'+leaf_id+' .headway-visual-editor-input').serialize();
		...
		hwjs.post( headway_blog_url + '/?headway-process=edit-leaf&leaf-id=' + leaf_id_int, { encoded: headway_encode64(leaf_data) }, function(){
			hwjs('div#leaf-' + leaf_id_int + ' div.leaf-content').load(headway_blog_url + '/?headway-process=get-leaf-content&leaf-id=' + leaf_id_int + '&page-id=' + hwjs('input#current-page').val(), function(){
...
function load_leaf_options(leaf_id, leaf_type, hide){		
	...
	hwjs.ajax({
		url: headway_blog_url+'/?headway-process=get-leaf-options-width&leaf=' + leaf_type + '&id=' + leaf_id,
		async: false,
		success: function (data) {				
	...
	hwjs('div#control-' + leaf_id).load(headway_blog_url+'/?headway-process=leaf-options&leaf=' + leaf_type + '&id=' + leaf_id, false, function(){
		prepare_leaf_options(leaf_id, leaf_options_width);		
		
		hwjs.getScript(headway_blog_url+'/?headway-process=options-js&leaf=' + leaf_type + '&id=' + leaf_id);
	});
...
function headway_visual_editor_leafs(){
	...
	hwjs('a#load-template').click(function(){
		...
		hwjs('div#ve-working-overlay').show().animate({'opacity':1}, 250, false, function(){
			hwjs.ajax({
				url: headway_blog_url+'/?headway-process=load-template&page=' + escape(current_page) + '&template-name='+escape(template_name + '-' + template_id),
				success: function(data){
	...
	hwjs('a#save-template-submit').click(function(){
		...
		hwjs.ajax({ 
			url: headway_blog_url+'/?headway-process=save-leaf-template&template-name=' + escape(template_name) + '&page=' + escape(current_page) + '&columns=' + escape(columns) + '&column-1-width=' + escape(column_1_width) + '&column-2-width=' + escape(column_2_width) + '&column-3-width=' + escape(column_3_width) + '&column-4-width=' + escape(column_4_width), 
	...
	hwjs('a#delete-template-link').click(function(){
		...
		if(confirm('Are you sure?') === true){			
			hwjs.ajax({
				url: headway_blog_url+'/?headway-process=delete-template&template-id=' + template_id + '&template-name=' + template_name,
				async: false,
				success: function(data){	
	...
	hwjs('a#save-template-settings-button').click(function(){
		...
		hwjs.ajax({
			url: headway_blog_url+'/?headway-process=rename-template&id=' + template_id + '&template-name=' + template_name + '&new-name=' + rename_input.val(),
			async: false,
			success: function(data){	
	...
	hwjs('a#set-template-as-default').click(function(){
		...
		hwjs.ajax({
			url: headway_blog_url+'/?headway-process=set-default-leafs-template&id=' + template_id + '&template-name=' + template_name,
			async: false,
			success: function(data){
	...
	hwjs('a#remove-template-as-default').click(function(){
		...
		hwjs.ajax({
			url: headway_blog_url+'/?headway-process=remove-default-leafs-template',
			async: false,
			success: function(data){
				hwjs('small#default-template').remove();	
	...
	hwjs('a#export-template-button').click(function(){		
		...
		url = headway_blog_url + '/?headway-process=export-leaf-template&template-id='+escape(template_name + '-' + template_id) + '&template-name='+escape(template_name);
